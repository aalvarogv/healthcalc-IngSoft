...
public class HealthHospitalAdapter  implements HealthHospital {
    ...
    public int pesoIdeal(Gender genero, float altura) {
        ...
        try {
            double pesoIdeal = calc.getIdealBodyWeight(new Usuario((int) alturacm, 1, 1, genero));
    ...
    public double bmr(Gender genero, int edad, float altura, int peso) {
        ...
        try {
            double bmr = calc.basalMetabolicRate(new Usuario((int) alturacm, pesokg, edad, genero));
            return (double) bmr;
